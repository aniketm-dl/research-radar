...
class Summarizer:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-1.5-flash", temperature: float = 0.2):
        """
        Initialize the summarizer with Google Gemini.

        Args:
            api_key: Gemini API key (uses environment variable if not provided)
            model: Model to use for summarization (gemini-1.5-flash or gemini-1.5-pro)
